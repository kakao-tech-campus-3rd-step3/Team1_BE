...
public class WebPushTestController {
    ...
    @Operation(
        summary = "특정 MemberId 기준 WebPush 구독 등록",
        description = "요청 body에 memberId와 WebPush 구독 정보를 전달하면 해당 멤버 기준으로 등록합니다.",
        requestBody = @RequestBody(
            required = true,
            description = "MemberId와 WebPush 구독 정보",
            content = @Content(schema = @Schema(implementation = WebPushRegisterByMemberIdRequest.class))
        ),
        responses = {
            @ApiResponse(responseCode = "200", description = "구독 등록 성공"),
            @ApiResponse(responseCode = "404", description = "Member를 찾을 수 없음"),
            @ApiResponse(responseCode = "500", description = "서버 내부 오류")
        }
    )
    @PostMapping("/subscribeByMemberId")
    public SubscribeResponse subscribeByMemberId(
        @org.springframework.web.bind.annotation.RequestBody WebPushRegisterByMemberIdRequest request
    ) {
    ...
    @Data
    @AllArgsConstructor
    public static class WebPushRegisterByMemberIdRequest {

        private UUID memberId;
        private WebPushRegisterRequest.Keys keys;
